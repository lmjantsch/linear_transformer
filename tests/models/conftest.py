...
def pytest_generate_tests(metafunc):
    if "model_id" in metafunc.fixturenames and "config" in metafunc.fixturenames:
        params = []
        ids = []
        ...
        metafunc.parametrize("model_id, config", params, ids=ids, scope="session")
...
def patched_model_cache(
    model_id: str, config: dict, device: torch.device, dtype: torch.dtype, batch_inputs: dict
) -> dict:
    cache = {}
    model = AutoModelForCausalLM.from_pretrained(
        model_id, torch_dtype=dtype, attn_implementation='eager'
...
def pytest_sessionfinish(session: pytest.Session, exitstatus: int) -> None:
    ...
    # --- config tests (conservation) ---
    if hasattr(pytest, "_patched_model_kl_identity"):
        print_kl_table(pytest._patched_model_kl_identity)

    if hasattr(pytest, "_patched_model_forward_identity"):
        print_forward_identity_table(pytest._patched_model_forward_identity)
        for mid, data in pytest._patched_model_forward_identity.items():
            if extended or not data["passed"]:
                print_forward_table(mid, data["errors"])

    if hasattr(pytest, "_patched_model_embedding_ratio"):
        print_conservation_embedding_table(pytest._patched_model_embedding_ratio)

    if hasattr(pytest, "_patched_model_conservation_results"):
        print_conservation_summary_table(pytest._patched_model_conservation_results)
        if extended:
            for mid, data in pytest._patched_model_conservation_results.items():
