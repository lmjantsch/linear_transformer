...
This library lets you swap the backward pass of a HuggingFace transformer in-place, replacing standard `nn.Module` subgraphs with `Modular*` wrappers that implement a gradient-based attribution method that modifies only the backward pass while keeping the forward pass numerically identical.
...
### Modular operations (`src/modular_transformer/modules/`)
...
- `tests/modules/` — unit tests for individual `autograd.Function` classes. Key properties: forward identity with `F.*` equivalents, gradient shape, and conservation (`(x · ∇x).sum()` preserved through the op).
- `tests/models/` — integration tests that load real HF models, patch them, run a forward+backward pass, and check: (1) hidden-state identity (`rel_l2 < 1e-2`), (2) KL divergence of logits (`< 5e-3`), (3) conservation per-module (`< 0.1`), and (4) total attribution at the embedding layer matching the loss.
