...
@modular_ctx
def ig_mul(x: torch.Tensor, y: torch.Tensor, x_base: torch.Tensor | None = None, y_base: torch.Tensor | None = None) -> torch.Tensor:
    x_base = x_base if x_base is not None else torch.zeros_like(x)
    y_base = y_base if x_base is not None else torch.zeros_like(y)
    return DynamicBilinearMul.apply(x, y, x_base, y_base)
...
@modular_ctx
def ig_matmul(x: torch.Tensor, y: torch.Tensor, x_base: torch.Tensor | None = None, y_base: torch.Tensor | None = None) -> torch.Tensor:
    x_base = x_base if x_base is not None else torch.zeros_like(x)
    y_base = y_base if x_base is not None else torch.zeros_like(y)
    return DynamicBilinearMatmul.apply(x, y, x_base, y_base)
